...
export const initializeBulletproofSystem = async (): Promise<boolean> => {
  try {
    console.log('🔧 Initializing bulletproof persistence system...');
    ...
    if (tablesError) {
      console.warn('Could not check for tables:', tablesError);
      return false;
    }

    if (!tables || tables.length === 0) {
      console.log('📦 Creating user_data_snapshots table...');
      ...
      if (createError) {
        console.warn('Failed to create table with RPC, trying direct approach:', createError);
        ...
        try {
          ...
          console.log('✅ Table creation attempted via direct API');
        } catch (directError) {
          console.warn('Direct table creation also failed:', directError);
          console.log('📝 Please manually create the table using the SQL in createUserDataSnapshotsTable.sql');
        }
      } else {
        console.log('✅ user_data_snapshots table created successfully');
      }
    } else {
      console.log('✅ user_data_snapshots table already exists');
    }
    ...
    if (testError) {
      console.warn('Table exists but might not be accessible:', testError);
      return false;
    }

    console.log('✅ Bulletproof persistence system initialized successfully');
    return true;
  } catch (error) {
...
export const performEmergencyDataRecovery = async (userId: string): Promise<void> => {
  try {
    console.log('🚨 Performing emergency data recovery for user:', userId);
    ...
    try {
      ...
      if (!snapshotError && snapshot?.data_snapshot) {
        ...
        console.log('✅ Found data snapshot in Supabase');
      }
    } catch (error) {
      console.log('ℹ️ No data snapshot found in Supabase');
    }
    ...
    try {
      ...
      console.log('ℹ️ No enrollments found in database');
    }

    // Step 3: Merge and restore data
    if (recoveredData || enrollmentsData.length > 0) {
      console.log('🔄 Restoring recovered data to localStorage...');
      ...
      console.log('ℹ️ No data to recover - user might be new or data was already cleared');
    }
  } catch (error) {
...
export const restoreCommonCourses = (userId: string): void => {
  ...
  console.log('🎯 Restoring common course enrollments as fallback...');
  ...
  console.log('✅ Common course enrollments restored');
};
